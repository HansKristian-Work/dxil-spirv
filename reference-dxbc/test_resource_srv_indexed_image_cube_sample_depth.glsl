...
void main()
{
    ...
    SV_TARGET = vec4(texture(nonuniformEXT(samplerCubeShadow(_9[_42], _19[_42])), vec4(vec3(TEXCOORD.x, TEXCOORD.y, TEXCOORD.z), DEPTH_REF))).x;
    SV_TARGET_1 = vec4(textureGrad(nonuniformEXT(samplerCubeShadow(_9[_42], _19[_42])), vec4(vec3(TEXCOORD.x, TEXCOORD.y, TEXCOORD.z), DEPTH_REF), vec3(0.0), vec3(0.0))).x;
}
