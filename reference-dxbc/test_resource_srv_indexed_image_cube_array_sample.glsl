...
void main()
{
    ...
    float _119 = dFdx(TEXCOORD_2.x);
    float _120 = dFdy(TEXCOORD_2.x);
    float _123 = dFdx(TEXCOORD_2.y);
    float _124 = dFdy(TEXCOORD_2.y);
    float _127 = dFdx(TEXCOORD_2.z);
    float _128 = dFdy(TEXCOORD_2.z);
    vec4 _131 = textureGrad(nonuniformEXT(samplerCubeArray(_9[_46], _19[_46])), vec4(TEXCOORD.x, TEXCOORD.y, TEXCOORD.z, LAYER), vec3(_119, _123, _127), vec3(_120, _124, _128));
    float _135 = _131.x;
    SV_TARGET_4.x = _135;
