...
void main()
{
    ...
    SV_TARGET = vec4(texture(nonuniformEXT(sampler2DArrayShadow(_9[_45], _19[_45])), vec4(vec3(TEXCOORD.x, TEXCOORD.y, LAYER), DEPTH_REF))).x;
    SV_TARGET_1 = vec4(textureOffset(nonuniformEXT(sampler2DArrayShadow(_9[_45], _19[_45])), vec4(vec3(TEXCOORD.x, TEXCOORD.y, LAYER), DEPTH_REF), ivec2(-1, 0))).x;
    SV_TARGET_2 = vec4(textureGrad(nonuniformEXT(sampler2DArrayShadow(_9[_45], _19[_45])), vec4(vec3(TEXCOORD.x, TEXCOORD.y, LAYER), DEPTH_REF), vec2(0.0), vec2(0.0))).x;
}
